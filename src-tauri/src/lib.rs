...
fn setup_app(app: &mut tauri::App, repos: storage::ReposConfig) -> Result<(), Box<dyn std::error::Error>> {
    log::info!("[updater] endpoint: {}", updater_endpoint::ENDPOINT);
    ...
    let active_workspaces: Vec<crate::workspace_entry::WorkspaceEntry> = repos_ref.workspaces.clone();
    drop(repos_ref);
    startup_sync::sync_all_repos_on_startup(&active_repos, &|_pid| false, &|_key| false);
    app_lifecycle::spawn_startup_account_sync(handle.clone(), active_repos, active_workspaces);
