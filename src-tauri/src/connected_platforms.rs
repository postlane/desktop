...
pub fn list_connected_platforms(
    ...
    let (config_path, cred_id) = crate::config_paths::resolve_config_and_cred_id(&repo_id, &state)?;
    let mastodon_active = crate::config_paths::read_project_id_from_config(&config_path)
        .map(|project_id| {
            app.keyring()
