...
const MOCK_ORGS: OrgSummary[] = [
  { login: 'hugoelliott', display_name: 'Hugo Elliott', avatar_url: 'https://avatars.githubusercontent.com/u/1', is_personal: true, has_project: false },
  { login: 'postlane', display_name: 'Postlane', avatar_url: 'https://avatars.githubusercontent.com/orgs/postlane', is_personal: false, has_project: true },
];
...
describe('ModalOrgPicker — loading and display', () => {
  ...
  it('shows Free badge and $5/month badge', async () => {
    render(<ModalOrgPicker onNext={vi.fn()} onBack={vi.fn()} onPricingGate={vi.fn()} />);
    await waitFor(() => expect(screen.getByText('hugoelliott')).toBeDefined());
...
describe('ModalOrgPicker — selection and creation', () => {
  it('selecting org auto-fills workspace name', async () => {
    render(<ModalOrgPicker onNext={vi.fn()} onBack={vi.fn()} onPricingGate={vi.fn()} />);
    ...
    expect(input.value).toBe('Postlane');
  });
  ...
  it('create_project called with providerOrgLogin for org account', async () => {
    const onNext = vi.fn();
