...
describe('ModalScheduler — picker', () => {
  it('test_renders_provider_options_and_skip', () => {
    ...
    expect(screen.getByRole('button', { name: /outstand/i })).toBeDefined();
  ...
  it('test_selecting_outstand_opens_key_entry', async () => {
    render(<ModalScheduler {...defaultProps} />);
    await userEvent.click(screen.getByRole('button', { name: /outstand/i }));
    expect(screen.getByRole('textbox')).toBeDefined();
  });
...
describe('ModalScheduler — after connecting first provider', () => {
  ...
  it('test_second_provider_button_remains_enabled', async () => {
    await connectZernio();
    expect((screen.getByRole('button', { name: /outstand/i }) as HTMLButtonElement).disabled).toBe(false);
  });
});
