  ...
  return (
    <section className="mb-8 rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-700 dark:bg-zinc-900">
      <div className="mb-3 flex items-center gap-2">
        ...
        <span
          title="Edit rate = how often you changed the draft before approving. Does not account for dismissed posts."
                ...
                <span className={s.limited_data ? 'text-zinc-400' : 'text-zinc-700 dark:text-zinc-300'}>
                  ...
                  {s.limited_data && (
                    <span className="ml-2 text-zinc-400">
                      Limited data ({s.total_posts} posts)
                    </span>
                  )}
                </span>
                <span className={s.limited_data ? 'text-zinc-400' : 'text-zinc-600 dark:text-zinc-400'}>
                  {pct}% edited ({s.total_posts} posts)
                </span>
              </div>
              <div className="h-2 w-full rounded-full bg-zinc-100 dark:bg-zinc-800">
                <div
                  className={`h-2 rounded-full ${s.limited_data ? 'bg-zinc-300 dark:bg-zinc-600' : 'bg-zinc-700 dark:bg-zinc-300'}`}
...
export default function AllReposPublishedView({ onNavigateToRepo }: Props) {
  ...
  const loadPage = useCallback(async (pageIndex: number, append: boolean) => {
    try {
      const [result, modelStats] = await Promise.all([
        invoke<PublishedPost[]>('get_all_published', {
  ...
  // Cmd+H / Ctrl+H handled at App level
    ...
    return (
      <div className="flex h-full items-center justify-center p-8">
        <p className="text-center text-sm text-zinc-500">
          No posts published yet. Draft your first post with{' '}
    ...
    <div className="p-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {exportStatus && (
            <span className="text-xs text-zinc-500">{exportStatus}</span>
          )}
          ...
          {sentPosts.map((post) => {
            ...
            return (
              <TableRow key={`${post.repo_id}-${post.post_folder}`}>
                <TableCell>
                  <button
                    onClick={() => onNavigateToRepo(post.repo_id)}
