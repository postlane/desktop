...
function groupAndSort(posts: DraftPost[]): RepoGroup[] {
  ...
  // Within each group: failed first, then ready; newest created_at first within each
    ...
    <div className="flex h-full items-center justify-center p-8">
      <div className="max-w-sm text-center">
        ...
        <div className="mb-4 flex items-center justify-center gap-3 rounded-lg bg-zinc-100 px-4 py-3 dark:bg-zinc-800">
          ...
          <Button plain onClick={handleCopy} aria-label="Copy /draft-post command">
...
export default function AllReposDraftsView({ postWizardNudge, onNudgeDismissed }: Props) {
  ...
  // Flat ordered list of all cards for keyboard nav
  const allCards = posts; // backend already sorts

  const refresh = useCallback(async () => {
    try {
      const result = await invoke<DraftPost[]>('get_all_drafts');
      setPosts(result);
    } catch (e) {
  ...
  async function handleApproveAll() {
    ...
    for (const post of ready) {
      try {
        ...
        setApproveAllResults((prev) => new Map(prev).set(post.post_folder, 'error'));
      }
    }
  ...
  // Wizard nudge overrides empty state
  if (postWizardNudge) {
    return <WizardNudge onDismiss={onNudgeDismissed} />;
  }
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between border-b border-zinc-200 px-6 py-4 dark:border-zinc-700">
        ...
        {readyCount >= 2 && (
          <Button color="green" onClick={() => setApproveAllOpen(true)}>
          ...
          <section key={group.repoId}>
            ...
            <div className="space-y-3">
              {group.posts.map((post) => {
                const globalIndex = allCards.indexOf(post);
                return (
                  <PostCard
      ...
      {/* Approve all dialog */}
      <Dialog open={approveAllOpen} onClose={() => setApproveAllOpen(false)}>
        <DialogTitle>Approve all ready posts</DialogTitle>
        <DialogDescription>
          Send {readyCount} post{readyCount !== 1 ? 's' : ''} to your scheduler?
        </DialogDescription>
        <DialogBody>
          {approveAllResults.size > 0 && (
            <ul className="space-y-1">
              {[...approveAllResults.entries()].map(([folder, result]) => (
        ...
        <DialogActions>
          <Button plain onClick={() => setApproveAllOpen(false)}>
            Cancel
          </Button>
          <Button
            color="green"
            onClick={handleApproveAll}
            disabled={approveAllRunning}
          >
            {approveAllRunning ? 'Sending…' : 'Confirm'}
          </Button>
        </DialogActions>
      </Dialog>
    </>
