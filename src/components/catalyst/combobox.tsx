...
export function Combobox<T>({
  ...
  options: T[]
  displayValue: (_value: T | null) => string | undefined
  filter?: (_value: T, _query: string) => boolean
  className?: string
  placeholder?: string
  autoFocus?: boolean
  'aria-label'?: string
  children: (_value: NonNullable<T>) => React.ReactElement
} & Omit<Headless.ComboboxProps<T, false>, 'as' | 'multiple' | 'children'> & { anchor?: 'top' | 'bottom' }) {
  const [query, setQuery] = useState('')
  const filteredOptions = query === ''
    ? options
    : options.filter((o) => filter ? filter(o, query) : displayValue(o)?.toLowerCase().includes(query.toLowerCase()))

  return (
    <Headless.Combobox {...props} multiple={false} virtual={{ options: filteredOptions }} onClose={() => setQuery('')}>
      <span data-slot="control" className={clsx([className, CONTROL_CLASS])}>
        <Headless.ComboboxInput
          ...
          className={clsx([className, INPUT_CLASS])}
        />
      ...
      <Headless.ComboboxOptions transition anchor={anchor} className={OPTIONS_CLASS}>
